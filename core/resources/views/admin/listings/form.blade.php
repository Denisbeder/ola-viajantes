...
<div class="row justify-content-center">
    <div class="col-sm-6">
        @if($action === 'edit')
            {!! Form::model($record, [
                'route' => ['listings.update', $record->id],
                'method' => 'put',
                'files' => true,
            ])
            !!}
            ...
            <div class="col d-flex justify-content-end">
                <div class="btn-group mB-25">
                    ...
                    <div class="dropdown-menu pt-0 pb-0">
                        ...
                        <button type="submit" name="submit_new" class="dropdown-item">Salvar e criar novo com sublista desse</button>
                        @empty(!$lastCreateId = request()->query('lc'))
                        <button type="submit" name="submit_new" value="{{ $lastCreateId }}" class="dropdown-item">Salvar e criar novo com sublista de {{ App\Listing::find($lastCreateId)->title }}</button>
                        @endempty
                    </div>
                    <a href="{{ session()->get('url.intended') ?? route('listings.index') }}" class="btn bg-white border">Cancelar</a>
                </div>
            </div>
        </div>
