<table id="dataTable" class="table table-hover border-bottom mb-0" cellspacing="0" width="100%" {{ ($touched = session()->get('touched')) ? 'data-touched='.$touched  : null }}>
    <thead>
        <tr>
            ...
            <th class="border-top-0">Aparelho</th>
            <th class="border-top-0">Posição</th>
    ...
    <tbody>
        ...
        <tr id="tr_{{ $record->id }}">
            ...
            <td class="align-middle">{!! $record->present()->deviceLabel !!}</td>
            <td class="align-middle">{!! $record->present()->positionSize !!}</td>
            <td class="align-middle">{!! $record->present()->publishLabel !!}</td>
            <td class="text-right">
                @include('admin._actions-btn', ['duplicate' => false, 'comments' => false, 'view' => false])
            </td>
        </tr>
        @endforeach
    </tbody>
